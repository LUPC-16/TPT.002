...
def LeerArchivo(Ruta):
    ...
    dfFinal['CFDI_UUID_FACTURA']=None
    dfFinal['CFDI_UUID_COMPLEMENTO_PAGO']=None
    dfFinal['MONTO_APLICADO']=None
    dfFinal['DIFERENCIA']=None
    ...
    for fichero in directorio.iterdir():
        ...
        if "PAGOS" in str(fichero) and "$" not in str(fichero):
            ...
            df_PagosActual.drop(['NUM','EMPRESA','ACCOUNT_NO','COMPANIA','NOMBRE','RFC','MONTO_PAGO','FECHA_LIQUIDADO','TB_MONTO','TB_IEPS','TB_IVA','HS_MONTO','HS_IEPS','HS_IVA','GO_MONTO','GO_IEPS','GO_IVA','TERCEROS','PENDIENTE','CANAL','CUENTA_BANCO','MONEDA','COUNTRY','UBICACION','Código Postal'],axis = 1, inplace = True)
                    ...
                    if(Bandera == False):
                        ...
                        if Tabla.shape[0] >= 1:
                            ...
                            for item in listaFacturas:
                                ...
                                Sumatoria=float(df_PagosActual['TPI_MONTO'][i])+float(df_PagosActual['TPI_IEPS'][i])+float(df_PagosActual['TPI_IVA'][i])+float(df_PagosActual['TPT_MONTO'][i])+float(df_PagosActual['TPT_IEPS'][i])+float(df_PagosActual['TPT_IVA'][i])
            ...
            for i in range(0,df_PagosActual.shape[0]):
                ...
                for j in range(0,len(ListaTablas)):
                    ...
                    if(Bandera == False):
                        ...
                        Break    
            print("COMIENZA ESCRITURA MX_NORTE")
            book = load_workbook(fichero)
            writer = pd.ExcelWriter(fichero, engine='openpyxl')
            writer.book = book
            pandas.io.formats.excel.header_style = None
            writer.sheets = dict((ws.title, ws) for ws in book.worksheets)
            dfFinal.to_excel(writer, 'MX_NORTE', startcol = 32,  index = False)
            writer.save()
