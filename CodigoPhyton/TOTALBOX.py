...
def LeerArchivo(Ruta):
    ...
    dfFinal['CFDI_UUID_FACTURA_TB']=None
    dfFinal['CFDI_UUID_COMPLEMENTO_PAGO_TB']=None
    dfFinal['MONTO_APLICADO_TB']=None
    dfFinal['DIFERENCIA_TB']=None
    ...
    for fichero in directorio.iterdir():
        ...
        if "UUIDS" in str(fichero.name) and "$" not in str(fichero):
            ...
            if (Fichero2[1]==Fichero3[1]):
               ...
               df_UuidTemp=pd.read_excel(fichero,sheet_name='TOTALBOX')
    ...
    for fichero in directorio.iterdir():
        ...
        if "PAGOS" in str(fichero) and "$" not in str(fichero):
            ...
            df_PagosActual.drop(['NUM','EMPRESA','ACCOUNT_NO','COMPANIA','NOMBRE','RFC','MONTO_PAGO','FECHA_LIQUIDADO','TPI_MONTO','TPI_IEPS','TPI_IVA','TPT_MONTO','TPT_IEPS','TPT_IVA','HS_MONTO','HS_IEPS','HS_IVA','GO_MONTO','GO_IEPS','GO_IVA','TERCEROS','PENDIENTE','CANAL','CUENTA_BANCO','MONEDA','COUNTRY','UBICACION','Código Postal'],axis = 1, inplace = True)
                ...
                for j in range(0,len(ListaTablas)):
                    ...
                    if(Bandera == False):
                        ...
                        if Tabla.shape[0] >= 1:
                            ...
                            Sumatoria=float(df_PagosActual['TB_MONTO'][i])+float(df_PagosActual['TB_IEPS'][i])+float(df_PagosActual['TB_IVA'][i])
            ...
            dfFinal.to_excel(writer, 'MX', startcol = 37,  index = False)
            writer.save()
    ...
    for fichero in directorio.iterdir():
        ...
        if "PAGOS" in str(fichero) and "$" not in str(fichero):
            ...
            pandas.io.formats.excel.header_style = None
            writer.sheets = dict((ws.title, ws) for ws in book.worksheets)
            dfFinal.to_excel(writer, 'MX_NORTE', startcol = 37,  index = False)
            writer.save()
            dfFinal.reset_index().to_csv('fichero.csv',header=True,index=False)         
            print('------------------------------------')
