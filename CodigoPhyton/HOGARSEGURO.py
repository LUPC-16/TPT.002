...
def LeerArchivo(Ruta):
    ...
    for fichero in directorio.iterdir():
        ...
        if "PAGOS" in str(fichero) and "$" not in str(fichero):
            ...
            df_PagosActual.drop(['NUM','EMPRESA','ACCOUNT_NO','COMPANIA','NOMBRE','RFC','MONTO_PAGO','FECHA_LIQUIDADO','TPI_MONTO','TPI_IEPS','TPI_IVA','TPT_MONTO','TPT_IEPS','TPT_IVA','GO_MONTO','GO_IEPS','GO_IVA','TB_MONTO','TB_IEPS','TB_IVA','TERCEROS','PENDIENTE','CANAL','CUENTA_BANCO','MONEDA','COUNTRY','UBICACION','Código Postal'],axis = 1, inplace = True)
